...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void decrease(int loc) {
		...
		T array1 = array[loc].getValue();
		T array2 = array[loc/2].getValue();
		if(array2.compareTo(array1)>0){
			ticker.tick();
			moveItem(loc, loc/2 );
			decrease(loc/2);
	...
	private void heapify(int where) {
		...
		if(right<=size){
			...
			child1=array[left].getValue();
			child2=array[right].getValue();
			father=array[where].getValue();
			ticker.tick();
			if(child1.compareTo(father)>0 && child2.compareTo(father)>0){
				ticker.tick();
				return;
			}
