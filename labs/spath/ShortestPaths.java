...
public class ShortestPaths {
	...
	public void run() {
		...
		//
		while(!pq.isEmpty()){
			VertexAndDist vertDist = pq.extractMin();
			Vertex verts = vertDist.getVertex();
			
			
			Iterable<Edge> edge = verts.edgesFrom();
			
			
			for(Edge edges: edge )
			{
				Decreaser<VertexAndDist> dec = map.get(edges.to);
				
				int current = dec.getValue().getDistance();
				int pot = weights.get(edges)+vertDist.getDistance();
			
				if(pot<current){
					dec.decrease(dec.getValue().sameVertexNewDistance(pot));
					toEdge.put(dec.getValue().getVertex(), edges);
				}
			}
		}
	}
	...
	public LinkedList<Edge> returnPath(Vertex endVertex) {
		...
		for(Vertex i= endVertex; i!=startVertex; i = toEdge.get(i).from)
		{
			path.addFirst(toEdge.get(i));
		}
		return path;
	}
